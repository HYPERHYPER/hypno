...
type OrgAccessAction = {
  updateOrganizations: (updatedOrganizations: OrgAccessState['organizations']) => void
...
const useOrgAccessStoreBase = create<OrgAccessState & OrgAccessAction>()(
  persist(
    (set, get) => ({
      ...
      isLoading: false,
      _hasHydrated: false,
      updateOrganizations: (updatedOrganizations) => set(() => ({ organizations: updatedOrganizations })),
      getOrganizations: async () => {
        const token = useUserStore.getState().token;
        if (_.isEmpty(get().organizations)) { set({ isLoading: true }) }
        try {
          const organizations : Organization[] = await fetchOrganizations(token?.access_token);
